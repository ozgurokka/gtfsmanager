...
// button actions
$("#saveAllTrips").on("click", function(){
	saveAllTrips();
});
...
function savePythonStopTimes(trip_id,time,stopTimes,list){
	...
	for( i in list) {
		var addTime = stopTimes[i].split(":");
		var dayA =  new Date(time.getTime());
		var dayD = new Date(time.getTime());
	
		dayA.setHours(time.getHours() + parseInt(addTime[0]));
		dayA.setMinutes(time.getMinutes() + parseInt(addTime[1]));
		dayA.setSeconds(time.getSeconds() + parseInt(addTime[2]));
		
		dayD.setHours(time.getHours() + parseInt(addTime[0]));
		dayD.setMinutes(time.getMinutes() + parseInt(addTime[1]));
		dayD.setSeconds(time.getSeconds() + parseInt(addTime[2]));
		...
		row['stop_id'] = list[i];
		row['timepoint'] = 0;
		row['arrival_time'] = dayA.toTimeString().split(' ')[0];
		row['departure_time'] = dayD.toTimeString().split(' ')[0];
		//row['shape_dist_traveled'] = '';
		timesArray.push({...row});
	}
	saveStopTimes(timesArray);
}
...
function saveStopTimes(data) {
	var pw = $("#password").val();
	if ( ! pw.length ) { 
		$('#timingsSaveStatus').html('<span class="alert alert-danger">Please enter the password.</span>');
		shakeIt('password'); return;
	}
	var trip_id = data[0].trip_id;
	var xhr = new XMLHttpRequest();
	xhr.open('POST', `${APIpath}tableReadSave?pw=${pw}&table=stop_times&key=trip_id&value=${trip_id}`);
	xhr.withCredentials = true;
	xhr.setRequestHeader('Content-Type', 'application/json; charset=utf-8');
...
function addTrip() {
	...
	if(delta === '')
	{
		alert("Sefer aralık bilgisini(delta) giriniz !")
		return;
	}
	if(trip_time === '')
	{
		alert("Sefer başlangıç zamanı bilgisini giriniz !")
		return;
	}
	if(trip_end_time === '')
	{
		alert("Sefer bitiş zamanı bilgisini giriniz !")
		return;
	}
	...
	var stopTimes = $('#stopTimes').val().split(/\r?\n/);
	...
	var dayStart = new Date("2022-01-01"+" "+trip_time);					
	var dayEnd = new Date("2022-01-01"+" "+trip_end_time) ;
	var dirIndex = 0;
	var tripStartTimes = [];
	var trips = [];
	...
	if(list.length != stopTimes.length){
		alert("Durak sayısı ile, durak varış zaman sayıları aynı değil !")
		return
	}

	for( var d = dayStart; d<=dayEnd ; d.setMinutes(d.getMinutes() + parseInt(delta))) { // looping through delta
		var trip = {};
		
		tripStartTimes.push(new Date(d.getTime()));
		...
		var trip_short_name = chosenRouteShortName + ' ' + d.toTimeString().split(' ')[0]  + ' ' + trip_headsign;
		var shape_id = '';
		...
		var trip_id = route_id+"_"+Math.random().toString(16).slice(7);
		...
		$("#trips-table").tabulator('addRow',{ route_id:route_id, trip_id: trip_id, 
			service_id:service_id, direction_id:'', shape_id:shape_id,
			trip_short_name: trip_short_name, trip_headsign:trip_headsign },true);
	}
	saveTrips();
	//save stop times
	for (var i = 0; i < tripStartTimes.length; i++){
		savePythonStopTimes(trips[i].trip_id,tripStartTimes[i],stopTimes,list);
	}
	alert("Sucess!")
