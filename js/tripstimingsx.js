...
function getPythonStopTimes(trip_id){
	...
	var refId = document.getElementById("tripNumber").value
	
	//referans girilmişse ordan sorgula, girilmemişse db'den boş getir.
	if(refId === ''){
		xhr.open('GET', `${APIpath}stopTimes?trip=${trip_id}&route=${chosenRoute}&direction=${chosenDirection}`);
	}else{
		xhr.open('GET', `${APIpath}stopTimes?trip=${refId}&route=${chosenRoute}&direction=${chosenDirection}`);
	}
	xhr.onload = function () {
		if (xhr.status === 200) { //we have got a Response
			...
			for (var i = 0; i < returndata.data.length; i++){
					var arriveTime = returndata.data[i].arrival_time;
					var departureTime = returndata.data[i].departure_time;
					var dayA = new Date("2022-01-01"+" "+arriveTime);					
					var dayD = new Date("2022-01-01"+" "+departureTime) ;
					dayA.setMinutes(dayA.getMinutes() + parseInt(delay));
					dayD.setMinutes(dayD.getMinutes() + parseInt(delay));
					returndata.data[i].arrival_time = dayA.toTimeString().split(' ')[0];
					returndata.data[i].departure_time = dayD.toTimeString().split(' ')[0];
					returndata.data[i].departure_time = dayD.toTimeString().split(' ')[0];
					returndata.data[i].trip_id = trip_id; 
			}
