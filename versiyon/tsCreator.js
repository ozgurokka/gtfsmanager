...
function savePythonStopTimes(trip_id,time,stopTimes){
...
function saveAllTrips() {
	...
	for( var d = dayStart; d<=dayEnd ; d.setMinutes(d.getMinutes() + parseInt(delta))) { // looping through delta
		...
		//trip.trip_id = route_id+"_"+Math.random().toString(16).slice(7);
		...
		trip.trip_id = route_id + pad(counter);
	...
	for (var i = 0; i < tripStartTimes.length; i++){
		savePythonStopTimes(trips[i].trip_id,tripStartTimes[i],stopTimes);
	}
